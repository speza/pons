...
### 1. The external runtime supports one capability: `tool_provider/v1`
...
Runtime protocol 1 defines exactly one capability, `tool_provider/v1`, for
hands-side tool execution. A connection advertises one tool-provider
capability and its catalog. Brain, session, event, and policy providers are
not capabilities of runtime protocol 1; the host rejects unsupported
...
### 3. External tool providers run on the hands side

A tool provider is launched with hands placement. The external adapter rejects
other placements. The child receives tool descriptions and actions and returns
`ToolResult` values; it does not receive the brain's conversation or a `Core`
handle.
...
- New capabilities require a new host-side contract and adapter rather than
  adding arbitrary methods at runtime.
